...
MODEL_SRT = "gpt-4-1106-preview"
MIN_OUTPUT_TOKENS = 300
SAMPLE_RATE = '8000'
TRANSCRIBE_ONLY = sys.argv[2] if len(sys.argv) > 2 else False


MODELS_INFO = {
    'gpt-3.5-turbo': {'max_tokens': 4097, 'per_1k_tokens_input': 0.0015, 'per_1k_tokens_output': 0.002},
    'gpt-3.5-turbo-16k': {'max_tokens': 16_385, 'per_1k_tokens_input': 0.003, 'per_1k_tokens_output': 0.004},
    'gpt-3.5-turbo-1106': {'max_tokens': 16_385, 'per_1k_tokens_input': 0.001, 'per_1k_tokens_output': 0.002},
    'gpt-4-1106-preview': {'max_tokens': 128_000, 'per_1k_tokens_input': 0.01, 'per_1k_tokens_output': 0.03},
    'gpt-4': {'max_tokens': 8192, 'per_1k_tokens_input': 0.03, 'per_1k_tokens_output': 0.06},
    'gpt-4-32k': {'max_tokens': 32_768, 'per_1k_tokens_input': 0.06, 'per_1k_tokens_output': 0.12},
...
def meeting_minutes(transcription, transcription_srt):
    system_texts = {
        ...
        "truthfulness": {"summary_type": "claim evaluation", "system": "Examine this claim and based on your knowledge of verifiable fact categorize it as TRUE, FALSE, SUBJECTIVE, or UNKNOWN—respond with classification - reason. Assign 'FALSE' with verifiable incorrect information and provide a brief correction. Use 'SUBJECTIVE' for opinions or interpretive statements, with a succinct explanation. If a statement cannot be verified with the training data, classify it as UNKNOWN. Ignore minor typos in names."},
        ...
        "tags": {"summary_type": "tags", "system": "You are an AI expert in analyzing text and extracting relevant tags. Please review the text and identify descriptive labels that accurately represent the tone, content, style, and target audience of the text. These tags should provide a quick overview of the text's main themes or topics. Please list these tags clearly and concisely. Do not explain or provide context for the tags."},
        "categories": {"summary_type": "categories", "system": f"You are an AI expert in analyzing text and categorizing it. Please review the text and assign a score between 0 and 5 for each category. A score of 0 means the category is not represented at all in the text, while a score of 5 means the category is a major theme of the text. Add the score and the category name to the list below in the format 'funny:3, irreverent:4, sci-fi:0 ...'\n{', '.join(CATEGORIES)}."},
    }

    abstract_summary = summary_extraction(transcription, system_texts['abstract']['summary_type'], system_texts['abstract']['system'])
    key_points = summary_extraction(transcription, system_texts['key_points']['summary_type'], system_texts['key_points']['system'])
    ...
    # tags = summary_extraction(transcription, system_texts['tags']['summary_type'], system_texts['tags']['system'])
    # categories = summary_extraction(transcription, system_texts['categories']['summary_type'], system_texts['categories']['system'])
    ...
    return {
        ...
        # 'tags'              : (tags['prompt_tokens'] * MODELS_INFO[MODEL]['per_1k_tokens_input'] + tags['completion_tokens'] * MODELS_INFO[MODEL]['per_1k_tokens_output']) / 1000,
        # 'categories'        : (categories['prompt_tokens'] * MODELS_INFO[MODEL]['per_1k_tokens_input'] + categories['completion_tokens'] * MODELS_INFO[MODEL]['per_1k_tokens_output']) / 1000,
    }
...
def transcribe_audio(files):
    ...
    #     minutes = open(
    #         file=OUTPUT_DIR + files['audio'] + '-minutes.md', mode='r'
    #     ).read()
    # else:
    minutes, costs = meeting_minutes(transcription=minimized_text, transcription_srt=None)
    ...
    with open(OUTPUT_DIR + f'{files["minutes"]}', 'w') as f:
        ...
        for title, text in minutes.items():
            if title == 'categories':
                ...
                text = re.sub(r', ', '\n- ', text)
                # calculate the maximum length of the category names
                max_length = max(len(x.split(':')[0]) for x in text.split('\n'))
                # substitute number with stars
                text = re.sub(r'(\w+[-\w ]*):(\d)', lambda x: f"{x.group(1)}:{' ' * (max_length - len(x.group(1)))} {'⭐' * int(x.group(2)) if int(x.group(2)) > 0 else ' '}", text)
                # remove periods
                text = re.sub(r'\.', '', text)
                # strip whitespace
                text = text.strip()
    ...
    print(f"All done! Find your files in {OUTPUT_DIR}")
    # total_cost = costs['abstract_summary'] + costs['key_points'] + costs['action_items'] + costs['sentiment'] + costs['tags'] + audio_cost
